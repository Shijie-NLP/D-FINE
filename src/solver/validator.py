...
class Validator:
        ...
        for pred, gt in zip(preds, self.gt):
            ...
            for label in labels:
                ...
                for pred_idx, gt_idx, iou in zip(pred_indices, gt_indices, iou_values):
                    if (
                        gt_idx.item() not in matched_gts
                        and pred_idx.item() not in matched_preds
                    ):
                ...
                metrics_per_class[label.item()]["IoUs"].extend(
                    [0] * len(unmatched_preds)
                )
    ...
    def _compute_metrics_and_confusion_matrix(self, preds):
        ...
        conf_matrix = np.zeros(
            (n_classes + 1, n_classes + 1), dtype=int
    ...
    def save_plots(self, path_to_save) -> None:
        ...
        if self.conf_matrix is not None:
            class_labels = [str(cls_id) for cls_id in self.class_to_idx.keys()] + [
                "background"
            ]
